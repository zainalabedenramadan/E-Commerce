...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'email', 'is_superuser', 'is_staff', 'password']
...
class ProductTypeSerializer(serializers.ModelSerializer):
    # استعلام مُحسن: prefetch_related للمنتجات
    ...
    def get_products(self, obj):
        # qs = obj.products.all().select_related('product_type', 'product_type__company')
        qs = obj.products.all()
        return ProductDetailSerializer(qs, many=True, context=self.context).data
...
class CompanySerializer(serializers.ModelSerializer):
    ...
    def get_product_types(self, obj):
        qs = obj.product_types.all().select_related('company').prefetch_related('products')
        return ProductTypeSerializer(qs, many=True, context=self.context).data
...
class AddCartItemSerializer(serializers.ModelSerializer):
    ...
    def save(self):
        ...
        try:
            # cartitem = CartItem.objects.select_related('product').get(product_id=product_id, cart_id=cart_id)
           with transaction.atomic():
            cartitem = CartItem.objects.select_for_update().select_related('product').get(product_id=product_id, cart_id=cart_id)
...
class Cartserializer(serializers.ModelSerializer):
    ...
    def main_total(self, cart: Cart):
        # prefetch_related already fetches items + products
...
class CreateOrderSerializer(serializers.Serializer):
    ...
    def save(self, **kwargs):
        with transaction.atomic():
            ...
            cartitems = CartItem.objects.select_for_update().select_related('product').filter(cart_id=cart_id)
